...
# Colonoscopy DenseNet121 (classification)
col_model = models.densenet121(weights=None)

# ✅ Match the classifier used during training
col_model.classifier = nn.Sequential(
    nn.Linear(col_model.classifier.in_features, 512),
    nn.ReLU(),
    nn.Dropout(0.5),
...
col_model.load_state_dict(torch.load(
    r"C:\Users\rishw\OneDrive\Desktop\datasets\densenet121\best_densenet121_balanced.pth",
    map_location=device
))
col_model = col_model.to(device).eval()
col_class_names = ["adenomatous", "hyperplastic"]


# Histopathology DenseNet121 (stage classification)
hist_model = models.densenet121(weights=None)  # ✅ also DenseNet121
hist_model.classifier = nn.Linear(hist_model.classifier.in_features, 10)  # adjust to # of classes
hist_model.load_state_dict(torch.load(
    r"C:\Users\rishw\OneDrive\Desktop\datasets\Histopathology_Split\best_densenet121_stage.pth",
    map_location=device
))
hist_model = hist_model.to(device).eval()
hist_class_names = ["Not_specified", "Stage_I", "Stage_II", "Stage_IIA", "Stage_IIB",
                    "Stage_III", "Stage_IIIA", "Stage_IIIB", "Stage_IIIC", "Stage_IV"]
...
# 🔹 Streamlit UI
# =====================
st.title("🩺 AI for Polyp Detection & Cancer Staging")
...
if page == "Home":
    ...
    st.header("🔎 Colonoscopy Analysis (YOLO + DenseNet121)")
    ...
    st.header("🧬 Histopathology Stage Classification (DenseNet121)")
